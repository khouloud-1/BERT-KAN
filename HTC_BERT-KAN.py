...
class KANLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.in_dim = input_shape[-1]  # Input dimension (e.g., 768 for BERT)
        # Define trainable weights for the KAN layer
        self.kan_weights = self.add_weight(
            shape=(self.out_dim, self.in_dim),  # Shape: (out_dim, in_dim)
    ...
    def call(self, x):
        ...
        # Expand x to match the output dimension
        # Shape: (batch_size, sequence_length, out_dim, in_dim)
        x_expanded = tf.tile(tf.expand_dims(x, axis=2), [1, 1, self.out_dim, 1])

        # Apply weights
        # Shape: (batch_size, sequence_length, out_dim, in_dim)
        output = x_expanded * self.kan_weights

        # Sum over the in_dim dimension to reduce to (batch_size, sequence_length, out_dim)
        output = tf.reduce_sum(output, axis=-1)

        return output


# Fine-tune BERT model with KAN
def create_fine_tuned_bert_kan_model(num_categories, num_super_categories):
    ...
    kan_out = KANLayer(out_dim=128)(bert_output)
    kan_out = tf.keras.layers.GlobalMaxPooling1D()(kan_out)

    # Output layers
    category_output = tf.keras.layers.Dense(num_categories, activation='softmax', name='category_output')(kan_out)
    super_category_output = tf.keras.layers.Dense(num_super_categories, activation='softmax', name='super_category_output')(kan_out)
...
model = create_fine_tuned_bert_kan_model(num_categories, num_super_categories)
